...
class FramingAnalyzer:
    ...
    def _analyze_subject_ratio(self, curr_kpts: Dict, ref_kpts: Dict,
        ...
        if curr_ratio < ref_ratio - 0.05:
            ...
            # 차이 정도에 따라 구체적 거리 계산
            ratio_diff_percent = (ref_ratio - curr_ratio) * 100

            # 거리 추정 (경험적 공식)
            if ratio_diff_percent < 10:
                distance_advice = "한 걸음"
            elif ratio_diff_percent < 20:
                distance_advice = "두세 걸음"
            else:
                distance_advice = "여러 걸음"

            action = f"인물을 더 크게: 피사체에 {distance_advice} 가까이 가세요"
            detailed_action = {
                'primary': f"카메라를 피사체 쪽으로 {distance_advice} 이동",
                'secondary': "또는 줌을 한 단계 확대 (압축감 약간 변화)",
                'ratio_diff': ratio_diff_percent,
                'method': 'move_closer'
            }
            ...
            ratio_diff_percent = (curr_ratio - ref_ratio) * 100

            if ratio_diff_percent < 10:
                distance_advice = "반 걸음"
            elif ratio_diff_percent < 20:
                distance_advice = "한두 걸음"
            else:
                distance_advice = "몇 걸음"

            action = f"인물을 더 작게: 피사체에서 {distance_advice} 멀어지세요"
            detailed_action = {
                'primary': f"카메라를 피사체에서 {distance_advice} 뒤로 이동",
                'secondary': "또는 줌을 한 단계 축소 (압축감 약간 변화)",
                'ratio_diff': ratio_diff_percent,
                'method': 'move_away'
            }
        else:
