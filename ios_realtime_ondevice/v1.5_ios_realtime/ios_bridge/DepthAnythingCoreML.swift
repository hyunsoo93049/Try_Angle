...
class DepthAnythingCoreML {
    ...
    enum ModelType {
        ...
        var modelName: String {
            switch self {
            case .small: return "DepthAnything_Small"
            case .base: return "DepthAnything_Base"
            }
        }
    }
    ...
    // MARK: - 깊이 추정
    func estimateDepth(from image: UIImage, completion: @escaping (Result<DepthResult, Error>) -> Void) {
    ...
    // MARK: - 깊이맵 처리
    private func processDepthMap(_ depthMap: MLMultiArray, originalImage: UIImage) -> DepthResult {
        // 압축감 계산
    ...
    // MARK: - 카메라 타입 판정
    private func determineCameraType(compression: Float) -> CameraType {
        switch compression {
        case ..<0.3:
            return .wide
        case 0.3..<0.5:
...
enum CameraType {
    ...
    var recommendation: String {
        ...
        case .normal: return "적절한 압축감"
        case .semiTele: return "좋은 압축감"
